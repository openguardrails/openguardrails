/**
 * AI Security Gateway - OpenRouter Chat Completions API handler
 *
 * Handles POST /v1/chat/completions requests routed to OpenRouter.
 * OpenRouter is OpenAI-compatible with extra optional attribution headers.
 */
...
export async function handleOpenRouterRequest(
  ...
  try {
    ...
    if (!backend) {
      ...
      res.end(JSON.stringify({ error: "OpenRouter backend not configured" }));
      return;
    }

    // 5. Forward to OpenRouter API
    ...
    if (stream) {
      await handleOpenRouterStream(response, res, mappingTable);
    } else {
      await handleOpenRouterNonStream(response, res, mappingTable);
    }
  } catch (error) {
    console.error("[ai-security-gateway] OpenRouter handler error:", error);
    res.writeHead(500, { "Content-Type": "application/json" });
    res.end(
      JSON.stringify({
