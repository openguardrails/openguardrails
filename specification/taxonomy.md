...
| `security.tainted_action` | An action with external effect whose CAUSAL CHAIN reaches untrusted content — the content itself need carry no instruction and the action need breach no [mandate](mandate.md). Distinct from `security.prompt_injection`, which judges one text on its merits: this judges an EDGE, and it is the control that still holds when the injected text carried nothing to detect. `security.memory_poisoning` is its cross-session special case. See [provenance](provenance.md). |
