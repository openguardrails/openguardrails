...
- **Bundle execution**: cloud-defined bundles (`bindingKind=bundle`) now actually cycle legs at request time. Each leg has optional `capUsdPerDay` / `capCallsPerDay`; the picker walks legs ascending by priority and skips any whose caps are exhausted by today's runs to that exact `(provider, model)` target. When all legs are exhausted, the tail leg is reused with an explicit "all legs exhausted" reason rather than a 503. Replaces the prior v1 stub that used `legs[0]` only. New module `src/policy/bundle.ts`; `PolicyConfig` widened to `CostCascadePolicy | BundlePolicy`.
- **`thomas cloud connect <agent>`**: routes a local agent through thomas-cloud's gateway instead of a local provider. Persists the `tc_gw_…` API key on `~/.thomas/cloud.json` and sets the agent's route to the sentinel `thomas-cloud` provider. The proxy detects the sentinel and forwards to `${cloud}/v1/messages` (or `/v1/chat/completions`) with the inbound body verbatim, adding `Authorization: Bearer <key>` + `X-Thomas-Agent-Id: <agent>`. Policy / bundle / failover and cross-protocol translation all happen on the cloud side; the local proxy stays in the loop only because model SDKs cannot inject the agent-id header themselves.
