...
class DetectionResultResponse(BaseModel):
    ...
    detection_scope: Optional[str] = None  # What was detected: 'all', 'last_message', 'user_only'
    sliding_window_count: Optional[int] = None  # Number of sliding windows used
    matched_window_indices: Optional[List[int]] = None  # Which windows matched
    # Audit: complete original messages array (system + user/assistant + tool_calls + tool results).
    # Only populated on detail endpoint to keep list responses small.
...
class PaginatedResponse(BaseModel):
    ...
    `total` and `pages` are optional: for queries where computing the exact
    count is too expensive (e.g., substring search across millions of rows),
    the server returns null and the caller falls back to `has_more` for
