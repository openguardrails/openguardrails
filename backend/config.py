...
class Settings(BaseSettings):
    ...
    # When set, the rate limiter and distributed locks use Redis instead of
    # PostgreSQL advisory locks / counter rows. When empty, services fall
    # back to the legacy PG-only behavior — this preserves smooth rollout
    # for existing deployments that haven't yet provisioned Redis.
    # Format: redis://[:password@]host:port/db  (or rediss:// for TLS)
    redis_url: str = ""
    # Optional separate password (overrides any password embedded in
    # redis_url). Useful when password is rotated independently of host.
    redis_password: str = ""
    # Key prefix to namespace OpenGuardrails keys when sharing a Redis
    # instance with other apps. All keys written by the platform are
    # prefixed with this string (default "og:").
    redis_key_prefix: str = "og:"
    # Connection pool size per worker. With ~8 async workers and pool=20,
    # peak concurrent Redis connections is ~160 — well within Redis defaults.
    redis_pool_size: int = 20
    # Socket timeout (seconds). Short to avoid blocking the event loop on
    # a degraded Redis. Operations that exceed this fail fast and the
    ...
    # Log retention (days). Detection log files older than this are auto-deleted.
    detection_log_retention_days: int = int(os.getenv("DETECTION_LOG_RETENTION_DAYS", "7"))
    ...
    api_domain: str = "https://api.openguardrails.com" if deployment_mode.lower() == "saas" else "http://localhost:50001"
