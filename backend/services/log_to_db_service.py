...
from database.models import DetectionResult, DetectionResultCategory, DetectionResultPayload
    ...
    async def _save_log_to_db(self, db: Session, log_data: dict) -> bool:
        ...
        try:
            ...
            detection_result = DetectionResult(
                ...
                hit_keywords=log_data.get('hit_keywords'),
                ...
                detection_scope=log_data.get('detection_scope'),
                sliding_window_count=log_data.get('sliding_window_count'),
                matched_window_indices=log_data.get('matched_window_indices'),
                full_messages=log_data.get('full_messages'),
                created_at=created_at
            )
            
            # Dual-write: relational projection of the three category JSON
            # columns. Lets the listing endpoint filter by category with a
            # portable JOIN instead of PG-only `JSONB.contains([cat])`. The
            # JSON columns above are still authoritative until the read
            # switch ships in a follow-up; both paths must produce the same
            # filter results during the bake-in.
            seen: set[tuple[str, str]] = set()
            for kind, raw in (
                ("security", log_data.get("security_categories")),
                ("compliance", log_data.get("compliance_categories")),
                ("data", log_data.get("data_categories")),
            ):
                if not raw:
                    continue
                for cat in raw:
                    ...
                    if key in seen:
                        continue
                    seen.add(key)
                    detection_result.categories.append(
                        DetectionResultCategory(kind=kind, category=str(cat))
                    )

            # Step 7.2 dual-write: heavy payload sibling. The legacy
            # columns on detection_results are still written above (until
            # readers migrate); this row makes the same data available
            # via the slim `detection_results` + `LEFT JOIN payloads`
            # access pattern, and gives the retention purger a clean
            # `DELETE FROM detection_result_payloads WHERE ...` target
            # instead of the wide UPDATE on detection_results.
            detection_result.payload = DetectionResultPayload(
                content=log_data.get("content"),
                original_content=log_data.get("original_content"),
                model_response=log_data.get("model_response"),
                full_messages=log_data.get("full_messages"),
                messages=log_data.get("messages"),
                image_paths=log_data.get("image_paths") or [],
                unsafe_segments=log_data.get("unsafe_segments") or [],
                doublecheck_categories=log_data.get("doublecheck_categories"),
                doublecheck_reasoning=log_data.get("doublecheck_reasoning"),
                matched_window_indices=log_data.get("matched_window_indices"),
            )
            ...
            db.flush()  # ensure detection_result.id is populated for the stats increment

            # Step 7.4: incremental rollup. Skip on stats failure so a
            # transient stats issue doesn't drop the detection row itself.
            try:
                ...
                increment_for_detection(
                    ...
                    application_id=detection_result.application_id,
                    created_at=detection_result.created_at,
                    security_risk_level=detection_result.security_risk_level,
                    compliance_risk_level=detection_result.compliance_risk_level,
                    data_risk_level=detection_result.data_risk_level,
                )
            except Exception as stats_err:
                logger.warning(
                    f"Stats rollup increment failed for request_id={log_data.get('request_id')}: {stats_err}"
                )
