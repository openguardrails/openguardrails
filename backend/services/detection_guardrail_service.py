...
def _sanitize_full_messages(messages: Any) -> Optional[List[Dict[str, Any]]]:
    """Serialize a messages list for audit logging.

    Preserves every role (system, user, assistant, tool) and every field
    (content, tool_calls, tool_call_id, name). Base64 image payloads are
    retained so the audit view can display the original image.
    """
    if not messages:
        return None
    ...
    for msg in messages:
        if hasattr(msg, "model_dump"):
            data = msg.model_dump(exclude_none=False)
        elif hasattr(msg, "dict"):
            data = msg.dict(exclude_none=False)
        elif isinstance(msg, dict):
            data = dict(msg)
        else:
            continue
        if "content" in data:
            data["content"] = _serialize_content(data["content"])
...
class DetectionGuardrailService:
    ...
    async def detect_messages(
        ...
        # Capture the original messages BEFORE filtering — this is what we want
        # to persist for audit (system prompts, tool_calls, tool results all kept).
        full_messages = _sanitize_full_messages(messages)

        # Convert dictionary format messages to Message objects
        # Only user/assistant messages participate in detection.
        # Skip system, tool, and messages with None content (e.g., assistant tool_calls stubs).
        message_objects = []
        for msg in messages:
    ...
    async def check_guardrails(
        ...
        # Capture the full messages array for audit BEFORE any truncation/filtering.
        # When called from detect_messages (proxy/gateway), the caller passes the
        # original dict-form messages so tool_calls / tool role / system are preserved.
        # When called directly via the /v1/guardrails endpoint, fall back to the
        # Pydantic Message objects (system messages and tool_calls aren't supported
        # there, but this keeps the audit field populated).
        if full_messages is None:
            full_messages = _sanitize_full_messages(request.messages)
        ...
        if not truncated_messages:
            ...
            return await self._handle_error(request_id, "", "No valid messages after truncation", tenant_id, application_id, source=source, user_id=user_id)
        
        # If application_id is not provided but tenant_id is, find the
        # tenant's default application. Phase 3: async DB lookup.
        if not application_id and tenant_id:
            try:
                from database.connection import get_async_detection_db_session
                from sqlalchemy import select

                tenant_uuid = uuid.UUID(str(tenant_id))
                async with get_async_detection_db_session() as db:
                    ...
                    default_app_id = res.scalar_one_or_none()
        ...
        try:
            ...
            # 2. Security/Compliance detection (must run before DLP)
            ...
            for msg in request.messages:
                if msg.role not in ('user', 'assistant'):
                    # Only user/assistant messages are detected.
                    # system/tool roles (and assistant tool_call stubs) are skipped.
                    if settings.debug:
                        content_preview = str(msg.content)[:100] if msg.content else ""
                        logger.info(f"[SLIDING_WINDOW DEBUG] Skipping {msg.role} message (not detected): {content_preview}...")
                    continue
                content = msg.content
                if content is None:
            ...
            # Execute scanner-based detection (new system) or fall back to legacy detection
            matched_scanner_tags = []  # Initialize for logging
            detection_scope = "all"  # Default: all messages detected
            sliding_window_count = None
            matched_window_indices = None

            if application_id:
                # Use new scanner detection system. Phase 3: AsyncSession +
                # AsyncScannerDetectionService — no more event-loop blocking
                # while scanner config is loaded.
                try:
                    from database.connection import get_async_detection_db_session
                    from services.scanner_detection_service import AsyncScannerDetectionService
                    ...
                    async with get_async_detection_db_session() as scanner_db:
                        scanner_service = AsyncScannerDetectionService(scanner_db)
                    ...
                    model_response = "scanner_detection"
                    detection_scope = detection_result.detection_scope
                    sliding_window_count = detection_result.sliding_window_count
                    matched_window_indices = detection_result.matched_window_indices

                    logger.info(f"Scanner detection complete: risk={detection_result.overall_risk_level}, matched_tags={matched_scanner_tags}, scope={detection_scope}")
            ...
            # 3. Check security/compliance disposal action to determine if DLP detection is needed
            # Disposal strategy: Security/Compliance first, DLP only if Security/Compliance passes
            # - If security/compliance action is block or replace, skip DLP detection (no need)
            # - If security/compliance action is pass, run DLP detection
            general_policy_action = await self._get_general_policy_action(
                application_id, compliance_result, security_result, detection_direction
            )

            if general_policy_action in ("reject", "replace"):
                # Security/compliance requires block or replace, skip DLP detection
                logger.info(f"Security/compliance action is {general_policy_action}, skipping DLP detection")
                data_result = DataSecurityResult(risk_level="no_risk", categories=[], detected_entities=[])
                data_anonymized_text = None
            else:
                # 4. Security/compliance passes, run DLP detection
                content_for_data_detection = self._extract_content_for_data_detection(truncated_messages, detection_direction)
                data_result, data_anonymized_text = await self._check_data_security(
                    content_for_data_detection, tenant_id, direction=detection_direction, application_id=application_id
                )

            # 5. Determine final action (combine security/compliance and DLP results)
            ...
            unsafe_segments = []  # Specific unsafe text segments identified by doublecheck
            if overall_risk_level != 'no_risk' and suggest_action in ['reject', 'replace']:
                try:
                    dc_result, dc_categories, dc_reasoning, dc_unsafe_segments = await self._perform_doublecheck(
                    ...
                    if dc_result is not None:
                        ...
                        unsafe_segments = dc_unsafe_segments or []
                        if dc_result == 'overturned_safe':
                            ...
                            logger.info(f"Doublecheck confirmed unsafe with {len(unsafe_segments)} segments for request {request_id}")
                ...
                try:
                    ...
                    if tenant_id:
                        try:
                            ...
                            async with get_async_detection_db_session() as db:
                                res = await db.execute(
                                    select(Tenant.language).where(Tenant.id == tenant_id)
                                )
                                lang = res.scalar_one_or_none()
                                if lang:
                                    appeal_language = lang
                        except Exception:
                            pass
            ...
            # Extract actual detected content based on detection_scope
            # This ensures log content matches what was actually detected
            detected_content = self._extract_detected_content(messages_dict, detection_scope)

            await self._log_detection_result(
                request_id, detected_content, compliance_result, security_result, data_result,
            ...
            return await self._handle_error(request_id, user_content, str(e), tenant_id, application_id, source=source, user_id=user_id)
    ...
    def _extract_content_for_data_detection(self, messages: List[Message], direction: str) -> str:
        ...
        Only user/assistant messages participate in detection; system and tool
        roles are excluded.
        ...
        if direction == "output":
            ...
            # For input detection, check user messages only (tool/system excluded)
            target_roles = {"user"}
    ...
    async def _check_data_security(self, text: str, tenant_id: Optional[str], direction: str = "input", application_id: Optional[str] = None) -> Tuple[DataSecurityResult, Optional[str]]:
        ...
        try:
            from database.connection import get_async_detection_db_session
            from services.data_security_service import AsyncDataSecurityService

            async with get_async_detection_db_session() as db:
                service = AsyncDataSecurityService(db)
                logger.info(f"Calling detect_sensitive_data for text: {text[:50]}...")
                result = await service.detect_sensitive_data(
                    text, tenant_id, direction, application_id=application_id
                )
    ...
    async def _determine_action_with_data(
        ...
        # Case 2: Only DLP risk (no general risk) - use data masking template with entity types
            ...
            # Get user's language preference for i18n. Phase 3: async lookup.
        ...
        suggest_action = await self._get_policy_action(application_id, general_risk_level, data_result, direction)
    ...
    async def _get_general_policy_action(
        self,
        ...
        """Get security/compliance disposal action only (not including DLP).

        This method is used to determine if DLP detection should be skipped.
        If security/compliance action is 'reject' or 'replace', DLP detection is not needed.

        Returns:
            'reject' | 'replace' | 'pass'
        """
        # Determine general risk level (security + compliance only)
        general_risk_levels = [compliance_result.risk_level, security_result.risk_level]
        general_risk_level = "no_risk"
        for level in ["high_risk", "medium_risk", "low_risk"]:
            if level in general_risk_levels:
        ...
        # If no general risk, return pass
        if general_risk_level == "no_risk":
            return "pass"

        # Get disposal action for general risk
        if not application_id:
            # No application context, use hardcoded defaults
        ...
        try:
            from database.connection import get_async_detection_db_session
            from services.data_leakage_disposal_service import AsyncDataLeakageDisposalService

            async with get_async_detection_db_session() as db:
                disposal_service = AsyncDataLeakageDisposalService(db)
                policy_action = await disposal_service.get_general_risk_action(
                    application_id=application_id,
            ...
            logger.warning(f"Failed to get general policy action: {e}, using defaults")
            if general_risk_level == "high_risk":
                return "reject"
            elif general_risk_level in ["medium_risk", "low_risk"]:
                return "replace"
    ...
    async def _get_policy_action(
        ...
        """Get final action from disposal policy.

        Priority logic:
        1. Check general risk (security/compliance) action first
        2. If reject or replace -> return immediately
        3. If pass -> check DLP action and return DLP result

        Maps policy actions to suggest_action values:
          General: block -> reject, replace -> replace, pass -> pass
          DLP: block -> reject, anonymize/anonymize_restore/switch_private_model/pass -> actual action
        """
        if not application_id:
            # No application context, use hardcoded defaults (backward compatibility)
            # 1. Check general risk first
            if general_risk_level == "high_risk":
                return "reject"
            elif general_risk_level in ["medium_risk", "low_risk"]:
                return "replace"
            # 2. General is pass, check DLP
            if data_result.risk_level == "high_risk":
                return "reject"
            elif data_result.risk_level in ["medium_risk", "low_risk"]:
                return "anonymize"
            return "pass"
        ...
        try:
            ...
            async with get_async_detection_db_session() as db:
                ...
                # 1. Check general risk action first (security/compliance)
                if general_risk_level != "no_risk":
                    policy_action = await disposal_service.get_general_risk_action(
                        application_id=application_id,
                        risk_level=general_risk_level,
                    ...
                    # If reject or replace, return immediately
                    if policy_action == "block":
                        return "reject"
                    elif policy_action == "replace":
                        return "replace"
                    # policy_action == "pass", continue to check DLP

                # 2. General is pass (or no_risk), check DLP action
                if data_result.risk_level != "no_risk" and data_result.detected_entities:
                    dlp_action = await disposal_service.get_disposal_action(
                        application_id=application_id,
                    ...
                    if dlp_action == "block":
                        return "reject"
                    # anonymize, anonymize_restore, switch_private_model, pass
                    return dlp_action
    ...
    async def _get_suggest_answer(self, categories: List[str], tenant_id: Optional[str] = None, application_id: Optional[str] = None, user_query: Optional[str] = None, matched_scanners: Optional[list] = None) -> str:
        ...
        # Get user's language preference. Phase 3: async DB lookup.
        user_language = None
        if tenant_id:
            try:
                from database.connection import get_async_detection_db_session
                ...
                async with get_async_detection_db_session() as db:
                    ...
                    user_language = res.scalar_one_or_none()
            except Exception as e:
    ...
    def _extract_detected_content(self, messages_dict: List[Dict], detection_scope: str) -> str:
        """
        Extract the actual content that was detected based on detection_scope.
        This ensures the logged content matches what was actually sent to the model.

        Args:
            messages_dict: List of message dicts (already filtered, no system messages)
            detection_scope: 'all', 'last_message', or 'user_only'

        Returns:
            String content that was actually detected
        """
        ...
        if detection_scope == "last_message":
            # Only the last message was detected
            return get_text(messages_dict[-1])
        elif detection_scope == "user_only":
            # Only user messages were detected
            user_texts = [get_text(m) for m in messages_dict if m.get("role") == "user"]
            return "\n".join(user_texts)
        else:
            # 'all' - all user/assistant messages were detected
            all_texts = []
            for msg in messages_dict:
                role = msg.get("role", "")
                text = get_text(msg)
                if text:
                    all_texts.append(f"[{role}]: {text}")
            return "\n".join(all_texts)
    ...
    async def _log_detection_result(
        ...
        unsafe_segments: Optional[List[str]] = None,
        detection_scope: Optional[str] = None,
        ...
        # Get workspace_id from application if available.
        # Phase 3: AsyncSession + select(), avoids the previous sync DB
        # transaction inside an async coroutine.
        workspace_id = None
        if application_id:
            try:
                from database.connection import get_async_admin_db_session
                from database.models import Application
                from sqlalchemy import select
                ...
                logger.warning(f"Failed to get workspace_id for application {application_id}: {e}")
        ...
        detection_data = {
            ...
            "doublecheck_categories": doublecheck_categories,
            "doublecheck_reasoning": doublecheck_reasoning,
            "unsafe_segments": unsafe_segments or [],
            # Detection scope fields (content already reflects actual detected content)
            "detection_scope": detection_scope,
            "sliding_window_count": sliding_window_count,
            "matched_window_indices": matched_window_indices,
    ...
    async def _perform_doublecheck(
        ...
        The AI must identify specific unsafe content segments. If it cannot, the detection is considered a false positive.

        Returns: (doublecheck_result, original_categories, reasoning, unsafe_segments)
            ...
            - unsafe_segments: list of specific unsafe text segments (only when confirmed_unsafe)
        """
        if not application_id:
            return None, None, None, None

        # Check if workspace has doublecheck enabled. Phase 3: async path
        # via workspace_resolver._async helper + async Workspace SELECT.
        try:
            from database.connection import get_async_detection_db_session
            from database.models import Workspace
            from services.workspace_resolver import get_workspace_id_for_app_async
            from sqlalchemy import select

            async with get_async_detection_db_session() as db:
                workspace_id = await get_workspace_id_for_app_async(db, application_id)
                if not workspace_id:
                    return None, None, None, None

                res = await db.execute(
                    select(Workspace.enable_doublecheck).where(Workspace.id == workspace_id)
                )
                enable_doublecheck = res.scalar_one_or_none()
                if not enable_doublecheck:
                    return None, None, None, None
        except Exception as e:
        ...
        # Build the doublecheck prompt - requires AI to identify specific unsafe content
...
The initial detection system flagged the following content as unsafe. Please carefully review and identify the SPECIFIC text segments that are actually unsafe.
...
4. If the content is actually risky, you MUST identify and quote the specific unsafe text segments

IMPORTANT: You MUST reply in EXACTLY the following JSON format:

If the content IS unsafe (detection correct), provide the specific unsafe segments:
{"is_unsafe": true, "unsafe_segments": ["exact quote of unsafe text 1", "exact quote of unsafe text 2"], "reasoning": "explanation of why these segments are unsafe"}

If the content is SAFE (false positive - detection wrong):
{"is_unsafe": false, "unsafe_segments": [], "reasoning": "explanation of why this is a false positive"}

Rules:
- is_unsafe=true means the content genuinely contains unsafe content AND you must provide the exact text quotes in unsafe_segments
- is_unsafe=false means the detection is wrong (content is actually safe)
- unsafe_segments MUST contain the EXACT text from the original content (direct quotes, not paraphrased)
- If you cannot identify specific unsafe text, the detection is considered a false positive
...
Initially detected risk categories: {categories_text}
Initially detected risk level: {overall_risk_level}

Review this content and identify the specific unsafe text segments (exact quotes). If you cannot find any specific unsafe content, mark it as a false positive."""
        ...
        try:
            ...
            if json_match:
                try:
                    ...
                    json_str = _re.sub(r',\s*}', '}', json_str)  # Remove trailing commas
                ...
                is_unsafe = parsed.get("is_unsafe", parsed.get("is_correct", False))
                # Handle backward compatibility: is_correct=true means unsafe
                if "is_correct" in parsed and "is_unsafe" not in parsed:
                    is_unsafe = parsed.get("is_correct", False)

                reasoning = parsed.get("reasoning", "")
                raw_segments = parsed.get("unsafe_segments", [])

                # Ensure raw_segments is a list of strings
                if isinstance(raw_segments, list):
                    raw_segments = [str(s) for s in raw_segments if s]
                else:
                    raw_segments = []

                # Convert text segments to format with position info for frontend highlighting
                unsafe_segments = []
                for segment_text in raw_segments:
                    # Find segment position in original content
                    start_pos = user_content.find(segment_text)
                    if start_pos >= 0:
                        unsafe_segments.append({
                            "text": segment_text,
                            "start": start_pos,
                            "end": start_pos + len(segment_text),
                        ...
                        start_pos = lower_content.find(lower_segment)
                        if start_pos >= 0:
                            # Use the original case from content
                            actual_text = user_content[start_pos:start_pos + len(segment_text)]
                            unsafe_segments.append({
                                "text": actual_text,
                                "start": start_pos,
                                "end": start_pos + len(segment_text),
                ...
                if is_unsafe and unsafe_segments:
                    # Content is unsafe AND we have specific segments with positions
                    return "confirmed_unsafe", all_categories, reasoning, unsafe_segments
                else:
                    # Either content is safe, or we couldn't identify/locate specific segments (treat as false positive)
                    if is_unsafe and not unsafe_segments:
                        reasoning = f"Unable to locate specific unsafe content in original text. {reasoning}"
                    return "overturned_safe", all_categories, reasoning, []
            else:
                # No valid JSON found - treat as false positive (cannot identify specific content)
                return "overturned_safe", all_categories, "Unable to parse response or identify specific unsafe content", []
    ...
    async def _handle_error(self, request_id: str, content: str, error: str, tenant_id: Optional[str] = None, application_id: Optional[str] = None, source: Optional[str] = None, user_id: Optional[str] = None) -> GuardrailResponse:
