...
from database.connection import get_admin_db_session, get_async_admin_db_session
...
class ProxyService:
    ...
    async def get_user_model_config(self, tenant_id: str, model_name: str) -> Optional[ProxyModelConfig]:
        ...
        Phase 3: async DB. The original sync version triggered a lazy
        load of `model.tenant` after the query and then `expunge`d the
        object — that pattern doesn't translate to AsyncSession (lazy
        loads are illegal). Instead we eager-load `tenant` via
        `selectinload` and detach the row from the session before
        returning, so callers can access attributes after the
        ...
        async with get_async_admin_db_session() as db:
            # First try exact config_name match.
            res = await db.execute(
                select(ProxyModelConfig)
                .where(
                    ProxyModelConfig.tenant_id == tenant_id,
                    ProxyModelConfig.config_name == model_name,
                    ProxyModelConfig.enabled == True,  # noqa: E712
                )
                .options(selectinload(ProxyModelConfig.tenant))
            )
            ...
            if model:
                # Detach so callers can read attributes after the
                # session closes. With expire_on_commit=False (our
                # default) attributes already loaded stay valid;
                # `expunge` just severs the identity-map link.
                db.expunge(model)
    ...
    async def log_proxy_request_gateway(
        ...
        Phase 3: AsyncSession — runs on every proxy request, so this was
        the highest-frequency sync DB write left in the proxy hot path."""
        try:
            async with get_async_admin_db_session() as db:
                log_entry = ProxyRequestLog(
                    request_id=request_id,
                    tenant_id=tenant_id,
                    upstream_api_config_id=upstream_api_config_id,
    ...
    async def log_proxy_request(
        ...
        """Log proxy request (legacy completions path). Phase 3: async."""
        try:
            async with get_async_admin_db_session() as db:
                log_entry = ProxyRequestLog(
                    request_id=request_id,
