...
from database.models import TenantSubscription, DetectionResult, Tenant, SubscriptionPayment
...
class BillingService:
    ...
    def check_and_increment_usage(self, tenant_id: str, db: Session) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if needs_reset:
                next_reset = self._get_next_reset_date(subscription, db, current_time)
    ...
    def get_subscription_with_usage(self, tenant_id: str, db: Session) -> Optional[dict]:
        ...
        try:
            ...
            if current_time >= subscription.usage_reset_at:
                ...
                subscription.usage_reset_at = self._get_next_reset_date(subscription, db, current_time)
                db.commit()
    ...
    def reset_all_quotas(self, db: Session) -> int:
        """Reset quotas for free-tier tenants whose anniversary cycle has elapsed.

        Subscribed tenants are reset by their Stripe billing cycle (handled in
        invoice.paid webhook + per-request _get_next_reset_date), so we skip
        them here to avoid overwriting their cycle-aligned usage_reset_at with
        a signup-anniversary date.
        """
        try:
            current_time = get_current_utc_time()
            next_reset = self._calculate_next_reset_date(current_time)

            # rowcount works on both PostgreSQL and MySQL — no need for
            # PG-only RETURNING just to learn how many rows we touched.
    ...
    def _get_next_reset_date(self, subscription: TenantSubscription, db: Session, current_time: datetime) -> datetime:
        """Pick the correct next quota reset date for a tenant.

        For subscribed tenants, the cycle is anchored to their Stripe billing
        cycle (subscription_payments.billing_cycle_end), so the free-period
        signup anniversary is ignored once they pay. Falls back to the
        signup-anniversary calculation for free tenants or when no active
        subscription_payment row exists.
        """
        if subscription.subscription_type == 'subscribed':
            ...
            if sub_payment and sub_payment.billing_cycle_end and sub_payment.billing_cycle_end > current_time:
                return sub_payment.billing_cycle_end
        return self._calculate_next_reset_date(current_time, subscription.created_at)
