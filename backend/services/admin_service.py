...
class AdminService:
    ...
    def get_all_users(self, db: Session, admin_tenant: Tenant, sort_by: str = 'created_at', sort_order: str = 'desc', skip: int = 0, limit: int = 20, search: Optional[str] = None) -> tuple[List[Dict[str, Any]], int]:
        ...
        # Only show independent tenants (self-registered, not created by any admin)
        # Exclude sub-users created by super admin (they belong to the same tenant space)
        base_query = base_query.filter(Tenant.created_by_tenant_id.is_(None))

        # Exclude current admin from the list (cannot switch to self)
        base_query = base_query.filter(Tenant.id != admin_tenant.id)
    ...
    def switch_to_user(self, db: Session, admin_tenant: Tenant, target_tenant_id: Union[str, uuid.UUID]) -> str:
        ...
        if admin_tenant.id == target_tenant_id:
            raise ValueError("Cannot switch to yourself")
        ...
        # Cannot switch to sub-users (they belong to the same tenant space)
        if target_tenant.created_by_tenant_id is not None:
            raise ValueError("Cannot switch to sub-users, only independent tenants are allowed")
