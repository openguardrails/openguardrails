...
class PaymentService:
    ...
    async def create_subscription_payment(
        ...
        try:
            if provider == 'alipay':
                ...
                # Update order with session ID. Dual-write: indexed column
                # for portable lookup, JSON for legacy readers (none currently
                # depend on it; safe to remove after one release of bake-in).
                payment_order.stripe_session_id = result['session_id']
    ...
    async def handle_subscription_paid(
        ...
        if subscription:
            # Subscription cycle now anchors the quota cycle: zero out the free-period
            # usage and reset the cycle to billing_end.
            subscription.current_month_usage = 0
            subscription.usage_reset_at = billing_end
        ...
        db.commit()
        billing_service._subscription_cache.pop(str(payment_order.tenant_id), None)
