...
class GuardrailService:
    ...
    async def check_guardrails(
        ...
        user_id: Optional[str] = None,  # End user ID (for syslog and ban policy)
        full_messages: Optional[List[Dict[str, Any]]] = None,  # Audit: complete original messages
    ) -> GuardrailResponse:
        ...
        # Capture full messages array for audit (system + tool_calls + tool results),
        # before any role/content filtering. Falls back to request.messages when the
        # caller didn't pass the original dict-form messages.
        from services.detection_guardrail_service import _sanitize_full_messages
        ...
        # Extract last user message only (for whitelist check to prevent bypass attacks)
        last_user_content = self._extract_last_user_content(request.messages)
        try:
            # 1. Blacklist/whitelist pre-check (using high-performance memory cache, application-scoped)
            # Blacklist: check entire conversation context (any turn with blacklisted keyword should block)
            ...
            # Whitelist: only check the LAST user message (not entire context)
            # This prevents bypass attacks where user includes whitelisted keyword in earlier turn
            # and sends malicious content in later turn
            whitelist_hit, whitelist_name, whitelist_keywords = await keyword_cache.check_whitelist(
                last_user_content, tenant_id=tenant_id, application_id=application_id
            )
            if whitelist_hit:
            ...
            for msg in request.messages:
                if msg.role not in ('user', 'assistant'):
                    continue  # Only user/assistant messages participate in detection
                content = msg.content
                if content is None:
                    continue
            ...
            return await self._handle_error(request_id, user_content, str(e), tenant_id, source=source, application_id=application_id, user_id=user_id)
    ...
    def _extract_last_user_content(self, messages: List[Message]) -> str:
        """Extract the last user message content only (for whitelist check)

        Whitelist should only check the last user message, not the entire conversation.
        This prevents bypass attacks where a user includes a whitelisted keyword in an
        earlier turn and then sends malicious content in a later turn.
        """
        for msg in reversed(messages):  # Get the last user message
            if msg.role == 'user':
                content = msg.content
                if isinstance(content, str):
                    return content
    ...
    def _get_policy_action(
        ...
        # Get data risk level safely
        data_risk_level = data_result.risk_level if data_result else "no_risk"
        has_detected_entities = data_result and data_result.detected_entities
        ...
        if not application_id:
            ...
            if data_risk_level == "high_risk":
                return "reject"
            elif data_risk_level in ["medium_risk", "low_risk"]:
                return "anonymize"
            return "pass"
        ...
        try:
            ...
            try:
                ...
                if data_risk_level != "no_risk" and has_detected_entities:
    ...
    async def _handle_error(self, request_id: str, content: str, error: str, tenant_id: Optional[int] = None, source: Optional[str] = None, application_id: Optional[str] = None, user_id: Optional[str] = None) -> GuardrailResponse:
