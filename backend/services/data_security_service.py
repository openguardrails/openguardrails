...
from services.workspace_resolver import (
    get_workspace_id_for_app, get_workspace_id_for_app_async,
    get_global_workspace_id, get_global_workspace_id_async,
)
...
def get_default_entity_types_config() -> List[Dict[str, Any]]:
    ...
    Chinese (zh): Chinese ID card, phone, name (genai), address (genai)
    English (en): (none - removed SSN and bank card due to false positives)
    ...
    # English-specific entity types (removed US SSN and bank card due to false positives)
    en_types = [
    ]
...
# ---------------------------------------------------------------------------
# Phase 3 — async sibling for the detection hot path.
#
# `DataSecurityService` is large (>2k LOC) but the detection chain only
# needs `detect_sensitive_data` plus its DB-touching helpers
# (`_get_user_entity_types` and the two `ensure_*_has_system_copies`
# methods). All admin CRUD methods (create/update/delete/disable/enable
# entity types, regex generators, test helpers) stay sync.
#
# The 5 overrides below are the only methods that touch `self.db`. The
# rest of the class — pattern matching, GenAI segmentation, anonymization
# (`_match_pattern`, `_match_pattern_genai`, `_detect_genai_with_segmentation`,
# `_anonymize_text_unified`, `_compare_risk_level`, etc.) — is pure
# compute or external HTTP, inherited unchanged.
# ---------------------------------------------------------------------------


class AsyncDataSecurityService(DataSecurityService):
    """Async variant of DataSecurityService for the detection hot path.

    Subclasses the sync class; only the 4 methods that touch the
    database are overridden. Everything else is inherited verbatim,
    keeping detection / anonymization / segmentation logic single-sourced.
    """

    def __init__(self, db: AsyncSession):
        # We can't `super().__init__(db)` because the parent type-hints
        # `db: Session` and we want `AsyncSession` here. Replicate the
        # parent's other field assignments manually so inherited methods
        # (`_match_pattern_genai`, `_detect_genai_with_segmentation`,
        # `_genai_anonymize_sync`, etc.) still find `self.model_service`.
        self.db = db
        self.model_service = ModelService()
    ...
    async def detect_sensitive_data(
        ...
        """Async detect_sensitive_data — same flow as the sync version,
        but calls the async `_get_user_entity_types`. The pattern-matching
        and anonymization helpers are inherited and untouched."""
        # 1. Format detection (pure — no DB).
        format_type = 'plain_text'
        format_metadata = {}
        if enable_format_detection:
            try:
        ...
        # 2. Async DB lookup for entity types.
        entity_types = await self._get_user_entity_types(tenant_id, direction, application_id)
        ...
        # 4. Regex pass (full text, no segmentation, pure compute).
        for entity_type in regex_entity_types:
            pattern_preview = entity_type.get('pattern', '')[:80] if entity_type.get('pattern') else 'NO PATTERN'
            logger.info(f"Regex checking entity type: {entity_type.get('entity_type')} with pattern: {pattern_preview}")
            matches = self._match_pattern(text, entity_type)
        ...
        # 5. GenAI pass (already-async helpers, model API call).
        if genai_entity_types:
            should_segment = (
                enable_smart_segmentation
                and format_type in ['json', 'yaml', 'csv', 'markdown']
                and len(text) > 1000
            )
            if should_segment:
                logger.info(f"Using smart segmentation for {format_type} format")
        ...
        # 6. Anonymization (pure compute, inherited helper).
        anonymized_text, restore_mapping = self._anonymize_text_unified(text, detected_entities, entity_types)
    ...
    async def _get_user_entity_types(
        self, tenant_id: str, direction: str, application_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Async variant. Resolves application → workspace, ensures
        system templates have been copied to the workspace, and returns
        the active entity-type definitions for the requested direction."""
        try:
            # Resolve workspace from application.
            workspace_id = None
            if application_id:
                workspace_id = await get_workspace_id_for_app_async(self.db, application_id)
                if not workspace_id:
                    logger.warning(
                        f"Could not resolve workspace for app {application_id}, "
                        f"falling back to global workspace"
                    )

            if not workspace_id:
                workspace_id = await get_global_workspace_id_async(self.db, tenant_id)

            if workspace_id:
                # Make sure all system templates have been mirrored to this
                # workspace (one-time backfill per workspace).
                await self.ensure_workspace_has_system_copies(tenant_id, workspace_id)

                # Disabled entity types for this workspace.
                disabled_entity_types = set()
                disabled_res = await self.db.execute(
                    select(TenantEntityTypeDisable).where(
                        and_(
                            TenantEntityTypeDisable.tenant_id == tenant_id,
                            TenantEntityTypeDisable.workspace_id == workspace_id,
                        )
                ...
                for disabled in disabled_res.scalars().all():
                    disabled_entity_types.add(disabled.entity_type)

                # Active entity types (system_copy + custom) at workspace level.
                res = await self.db.execute(
                    select(DataSecurityEntityType).where(
                        and_(
                            DataSecurityEntityType.is_active == True,  # noqa: E712
                            DataSecurityEntityType.workspace_id == workspace_id,
                        )
                    )
                )
                entity_types_orm = list(res.scalars().all())
            else:
                logger.warning(
                    f"No workspace resolved for tenant {tenant_id}, "
                    f"using tenant-level filtering"
                )
                await self.ensure_tenant_has_system_copies(tenant_id)
            ...
            # Same shape conversion as the sync version.
            entity_types: List[Dict[str, Any]] = []
            for et in entity_types_orm:
                if et.entity_type in disabled_entity_types:
                    continue
    ...
    async def ensure_workspace_has_system_copies(self, tenant_id: str, workspace_id: str) -> int:
        """Async variant. Mirrors all `system_template` entity types into
        the workspace as `system_copy` rows the first time. Idempotent:
        subsequent calls find the copies already present and no-op."""
        try:
            # All system templates.
            sys_res = await self.db.execute(
                select(DataSecurityEntityType).where(
                    DataSecurityEntityType.source_type == 'system_template'
                )
            )
            system_templates = list(sys_res.scalars().all())
            if not system_templates:
                return 0

            # Existing entity types in this workspace.
            ws_res = await self.db.execute(
                select(DataSecurityEntityType).where(
                    DataSecurityEntityType.workspace_id == workspace_id
                )
            )
            workspace_entity_types = list(ws_res.scalars().all())
            ...
            copies_created = 0
            for template in system_templates:
                if str(template.id) in workspace_template_ids:
                    continue
                recognition_config = template.recognition_config or {}
                copy = DataSecurityEntityType(
                    tenant_id=tenant_id,
                ...
                copies_created += 1
                logger.info(
                    f"Created system copy of '{template.entity_type}' for workspace {workspace_id}"
                )

            if copies_created > 0:
                await self.db.commit()
                logger.info(
                    f"Created {copies_created} system entity type copies for workspace {workspace_id}"
                )
            return copies_created
        except Exception as e:
            logger.error(f"Error ensuring workspace {workspace_id} has system copies: {e}")
    ...
    async def ensure_tenant_has_system_copies(self, tenant_id: str) -> int:
        """Resolve tenant → global workspace, then mirror system templates."""
        global_ws_id = await get_global_workspace_id_async(self.db, tenant_id)
        if not global_ws_id:
            logger.warning(
                f"Cannot ensure system copies: no global workspace for tenant {tenant_id}"
            )
            return 0
        return await self.ensure_workspace_has_system_copies(tenant_id, global_ws_id)
