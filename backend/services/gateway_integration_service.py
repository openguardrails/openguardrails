...
from services.data_leakage_disposal_service import DataLeakageDisposalService, AsyncDataLeakageDisposalService
...
class GatewayIntegrationService:
    ...
    async def process_input(
        ...
        try:
            ...
            # Extract detection_id and overall_risk from detection result
...
# ---------------------------------------------------------------------------
# Phase 3 — async sibling for the proxy / external-gateway hot path.
#
# `GatewayIntegrationService` only touches `self.db` in three places:
#   1. `_get_language` — single tenant SELECT
#   2. `__init__` — wraps `DataLeakageDisposalService(db)`
#   3. transitively, when calling `disposal_service.get_private_model(...)`
#      from inside `process_input` (line ~212)
#
# All other methods are pure logic, in-memory session bookkeeping, calls
# to `unified_anonymization_service`, the global `detection_guardrail_service`
# (which already manages its own async sessions internally), or `httpx`.
#
# The async subclass therefore overrides `__init__`, `_get_language`,
# `process_input`, and `process_output` — and inherits the rest verbatim.
# ---------------------------------------------------------------------------


class AsyncGatewayIntegrationService(GatewayIntegrationService):
    """Async variant of GatewayIntegrationService for the proxy hot path.

    Subclasses the sync class. Only the methods that touch the DB session
    are overridden; helpers like `_anonymize_messages`, `_create_session`,
    `_restore_content`, `_create_block_response`, `_create_replace_response`,
    `_create_switch_model_response`, and `_proxy_to_private_model` are
    inherited unchanged.
    """

    def __init__(self, db: AsyncSession):
        # Skip super().__init__ — the parent constructor would build a
        # sync DataLeakageDisposalService. Replicate the parent's
        # field assignments with the async equivalents.
        self.db = db
        self.disposal_service = AsyncDataLeakageDisposalService(db)

    async def _get_language(self, tenant_id: Optional[str]) -> str:
        """Async tenant.language lookup. Same fallback chain as the sync version."""
        if not tenant_id:
            return 'en'
        try:
    ...
    async def process_input(
        ...
        """Same flow as the sync version, with `await` on the two DB-touching
        calls (`_get_language` and `disposal_service.get_private_model`).

        The detection step itself is unchanged — `detection_guardrail_service`
        is a global singleton that manages its own async sessions internally
        (since wave 2). We only need to fix the DB calls bound to *this*
        service's session."""
        request_id = f"gw-{uuid.uuid4().hex[:12]}"

        try:
            language = await self._get_language(tenant_id)

            # 1. Run full detection (already async via the global instance).
            detection_result = await detection_guardrail_service.detect_messages(
                messages=messages,
                tenant_id=tenant_id,
                request_id=request_id,
            ...
            if data_risk_level != "no_risk" and detected_entities:
                ...
                if disposal_action == "reject":
                    ...
                    # Async DB lookup for the private model.
                    private_model = await self.disposal_service.get_private_model(
                        application_id=application_id,
                        tenant_id=tenant_id,
                    )
                    if private_model:
                    ...
                    # `_anonymize_messages` does not touch self.db — it
                    # delegates to `unified_anonymization_service`.
                    anonymized_messages, session_id, restore_mapping = self._anonymize_messages(
                        messages=messages,
                        detected_entities=detected_entities,
                        application_id=application_id,
    ...
    async def process_output(
        ...
        """Same flow as the sync version, with `await` on `_get_language`."""
        request_id = f"gw-out-{uuid.uuid4().hex[:12]}"
...
def get_async_gateway_integration_service(db: AsyncSession) -> AsyncGatewayIntegrationService:
    """Async factory — pass an AsyncSession from `get_async_*_db` deps."""
    return AsyncGatewayIntegrationService(db)
