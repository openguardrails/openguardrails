...
class ModelRouteService:
    ...
    # ------------------------------------------------------------------
    # Phase 3 — async variants for the proxy hot path. Same class,
    # `_async` suffix, since `ModelRouteService` is stateless (all methods
    # are @staticmethod). Sync callers (model_routes_api.py admin CRUD)
    # are unaffected.
    # ------------------------------------------------------------------

    @staticmethod
    async def find_matching_route_async(
        db: AsyncSession,
        tenant_id: str,
        model_name: str,
        application_id: Optional[str] = None,
    ) -> Optional[UpstreamApiConfig]:
        """Async variant of find_matching_route. See sync version for
        matching priority semantics. Eager-loads `route_applications`
        and `upstream_api_config` because both are read after the query
        and AsyncSession refuses to lazy-load."""
        try:
            tenant_uuid = uuid.UUID(tenant_id)
            ...
            res = await db.execute(
                ...
                .order_by(desc(ModelRoute.priority))
                .options(
                    selectinload(ModelRoute.route_applications),
                    selectinload(ModelRoute.upstream_api_config),
                )
            )
            routes_query = list(res.scalars().all())

            if not routes_query:
                logger.debug(f"No routes found for tenant {tenant_id}")
                return None

            # Bucket into application-specific vs global (identical to sync version).
            app_specific_routes: List[ModelRoute] = []
            global_routes: List[ModelRoute] = []
            ...
            if application_id and app_specific_routes:
                ...
                if match:
                    logger.info(
                        f"Model routing: '{model_name}' matched app-specific route '{match.name}' "
                        f"(pattern: {match.model_pattern}, type: {match.match_type})"
                    )
                    return match.upstream_api_config
            ...
            if global_routes:
                ...
                if match:
                    logger.info(
                        f"Model routing: '{model_name}' matched global route '{match.name}' "
                        f"(pattern: {match.model_pattern}, type: {match.match_type})"
                    )
                    return match.upstream_api_config
    ...
    @staticmethod
    async def get_routes_for_tenant_async(
        db: AsyncSession,
        tenant_id: str,
        include_inactive: bool = False,
    ) -> List[ModelRoute]:
        """Async variant of get_routes_for_tenant. Same eager-load set
        as `find_matching_route_async` so callers can read
        `route.route_applications` / `upstream_api_config` without
        triggering MissingGreenlet."""
        try:
            tenant_uuid = uuid.UUID(tenant_id)
            stmt = select(ModelRoute).where(ModelRoute.tenant_id == tenant_uuid)
            if not include_inactive:
                stmt = stmt.where(ModelRoute.is_active == True)  # noqa: E712
            stmt = stmt.order_by(desc(ModelRoute.priority)).options(
                selectinload(ModelRoute.route_applications),
                selectinload(ModelRoute.upstream_api_config),
            )
