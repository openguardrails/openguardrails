...
def get_admin_user(request: Request, db: Session) -> Tenant:
    """Get the original admin tenant for admin-only endpoints.

    When a super admin is impersonating another tenant via switch session,
    auth_context.data.tenant_id points to the impersonated user. Admin-only
    endpoints must check the original admin's permissions, so prefer
    original_admin_id when present.
    """
    ...
    tenant_id = data.get('original_admin_id') or data.get('tenant_id')
    if not tenant_id:
...
async def list_all_subscriptions(
    ...
    try:
        current_tenant = get_admin_user(request, db)
