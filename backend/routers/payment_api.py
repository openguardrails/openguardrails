...
async def verify_payment_session(
    ...
    try:
        ...
        # Look up by indexed columns (portable PG/MySQL); replaces the prior
        # PG-only `order_metadata->>'stripe_session_id'` text expression.
        order = db.query(PaymentOrder).filter(
            PaymentOrder.tenant_id == current_user.id,
            PaymentOrder.stripe_session_id == session_id,
        ).first()

        if not order:
            # Fall back to Alipay trade_no
            order = db.query(PaymentOrder).filter(
                PaymentOrder.tenant_id == current_user.id,
                PaymentOrder.trade_no == session_id,
            ).first()
...
async def stripe_webhook(
    ...
    try:
        ...
        if event_type == 'checkout.session.completed':
            ...
            if order_type == 'subscription':
                # Find order by session ID (indexed column lookup, portable PG/MySQL).
                from database.models import PaymentOrder
                order = db.query(PaymentOrder).filter(
                    PaymentOrder.stripe_session_id == session_data['session_id'],
                ).first()
            ...
            if subscription_id:
                ...
                if sub_payment:
                    ...
                    # Update tenant subscription (restore subscribed status + quota)
                    subscription = db.query(TenantSubscription).filter(
                        TenantSubscription.tenant_id == sub_payment.tenant_id
                    ).first()

                    if subscription:
                        # Restore subscription_type to 'subscribed' (in case it was downgraded)
                        subscription.subscription_type = 'subscribed'
                        subscription.subscription_expires_at = invoice_data.get('period_end')

                        # New billing cycle starts: zero out usage and align the
                        # quota reset to the cycle end so the dashboard matches Stripe.
                        period_end = invoice_data.get('period_end')
                        if period_end:
                            subscription.usage_reset_at = period_end
                        subscription.current_month_usage = 0
                        billing_service._subscription_cache.pop(str(sub_payment.tenant_id), None)

                        # Restore quota if currently on free tier (monthly_quota == free quota)
                        # This handles the case where subscription expired before webhook was processed
                        free_quota = billing_service.SUBSCRIPTION_CONFIGS.get('free', {}).get('monthly_quota', 1000)
                        current_tier = getattr(subscription, 'subscription_tier', 0) or 0

                        if subscription.monthly_quota == free_quota and current_tier > 0:
                            # Was downgraded to free, restore from tier config
                            tier_config = billing_service.get_tier_config(current_tier, db)
                            if tier_config:
                                subscription.monthly_quota = tier_config['monthly_quota']
                                logger.info(
                                    f"Restored quota for tenant {sub_payment.tenant_id} "
                                    f"to {subscription.monthly_quota} (tier {current_tier}) after invoice.paid"
                                )
                        elif subscription.monthly_quota == free_quota and current_tier == 0:
                            # Legacy subscription (tier 0) - restore legacy quota
                            tier_config = billing_service.get_tier_config(0, db)
                            if tier_config:
                                subscription.monthly_quota = tier_config['monthly_quota']
                                logger.info(
                                    f"Restored legacy quota for tenant {sub_payment.tenant_id} "
                                    f"to {subscription.monthly_quota} after invoice.paid"
                                )

                    db.commit()
                    logger.info(
                        f"Processed invoice.paid for subscription {subscription_id}: "
                        f"billing_cycle={invoice_data.get('period_start')}->{invoice_data.get('period_end')}"
                    )
