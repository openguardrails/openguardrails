...
a hosted [Runtime API](/api/docs/reference/).
...
One GuardEvent to [`POST /v1/evaluate`](/api/docs/reference/evaluate/).
...
[`POST /v1/ingest`](/api/docs/reference/ingest/) with `{"batch": [...]}`. At most
...
[`POST /v1/enroll`](/api/docs/reference/enroll/). `publicKey` is the base64url raw
32-byte Ed25519 public key.

### `heartbeat(extra?) -> Promise<{ ok: boolean }>`

[`POST /v1/heartbeat`](/api/docs/reference/heartbeat/). Pass the wire fields directly:
...
[`GET /v1/config`](/api/docs/reference/config/). The wire's `on_unreachable` arrives as
`config.onUnreachable`; other keys pass through.

### `getApproval(guardId) -> Promise<ApprovalStatus>`

[`GET /v1/approvals`](/api/docs/reference/approvals/). Resolves to
...
[`ogr-batch-signature`](/api/docs/reference/#request-signing-and-attestation) scheme
...
[enroll endpoint page](/api/docs/reference/enroll/#javascript--createnodesigner).
...
[`/v1/config`](/api/docs/reference/config/) policy when these fire.
