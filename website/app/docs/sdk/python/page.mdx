...
**`RuntimeClient`** for a hosted [Runtime API](/docs/api/). Models are
...
One GuardEvent to [`POST /v1/evaluate`](/docs/api/evaluate/); returns the
...
[`POST /v1/ingest`](/docs/api/ingest/). Accepts any iterable of `GuardEvent`
...
[`POST /v1/enroll`](/docs/api/enroll/). `public_key` is raw 32-byte Ed25519
...
[`POST /v1/heartbeat`](/docs/api/heartbeat/). Returns `{"ok": True}`.

### `get_config() -> dict`

[`GET /v1/config`](/docs/api/config/) — degraded-mode directives:
`{"on_unreachable": {"security.*": "block", ...}}`. Fetch at startup, cache,
refresh periodically.

### `get_approval(guard_id) -> dict`

[`GET /v1/approvals`](/docs/api/approvals/). Returns
...
the [`/v1/config`](/docs/api/config/) policy and must not default to allow
