...
[GuardEvent](/docs/api/objects/guard-event/) in, one
[Verdict](/docs/api/objects/verdict/) out. A PEP calls this when it is holding
...
envelope. Batching belongs to [`/v1/ingest`](/docs/api/ingest/). The runtime
...
[the GuardEvent object](/docs/api/objects/guard-event/) page):
...
| `ogr-batch-signature` | detached JWS | Raises the attestation ceiling — see [signing](/docs/api/#request-signing-and-attestation) |
...
The response body is a [Verdict](/docs/api/objects/verdict/) object: the
...
[degraded-mode](/docs/api/config/) policy from `/v1/config`. It must **not**
