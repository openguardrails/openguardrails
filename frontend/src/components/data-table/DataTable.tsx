...
interface DataTableProps<TData, TValue> {
  ...
  /**
   * For server-side pagination where the exact total is unknown (e.g.,
   * substring search over a huge table that bypasses COUNT). When set,
   * drives the next-button enable instead of computed pageCount, and the
   * "of N" suffix is hidden. Pass `pageCount={undefined}` (or 0) along
   * with this prop to opt in.
   */
  hasMore?: boolean
}
...
export function DataTable<TData, TValue>({
  ...
  // When the server signals an unknown total (pageCount undefined or 0)
  // and provides hasMore, we render in "next/prev only" mode without an
  // "of N" suffix and drive canGoNext from hasMore.
  const totalUnknown = !isClientSidePagination && hasMore !== undefined && (!pageCount || pageCount <= 0)
  ...
  const canGoNext = totalUnknown ? !!hasMore : actualCurrentPage < totalPages
  ...
  return (
    <div className={fillHeight ? "flex-1 flex flex-col min-h-0" : "space-y-4"}>
      <div className={`${fillHeight ? "flex-1 min-h-0 overflow-auto border-t" : "rounded-md border"} ${stickyLastColumn ? "overflow-x-auto overscroll-x-contain" : ""}`}>
      ...
      {pagination && (totalPages > 0 || totalUnknown) && (
        <div className={fillHeight ? "flex-shrink-0 flex items-center justify-between px-2 py-4 border-t bg-card" : "flex items-center justify-between px-2"}>
          ...
          <div className="flex items-center space-x-6 lg:space-x-8">
            <div className="flex w-[100px] items-center justify-center text-sm font-medium">
              {totalUnknown ? `Page ${actualCurrentPage}` : `Page ${actualCurrentPage} of ${totalPages}`}
            </div>
            <div className="flex items-center space-x-2">
              <Button
                variant="outline"
