...
export const resultsApi = {
  ...
  exportResults: (params?: {
    ...
    content_search?: string;
    request_id_search?: string;
    export_scope?: 'content' | 'full_messages';
  }): Promise<Blob> =>
...
// Step 7.3: global detection-data retention. Super-admin only.
...
export const retentionApi = {
  getConfig: (): Promise<RetentionConfig> =>
    api.get('/api/v1/system/retention').then(res => res.data),

  updateConfig: (config: RetentionConfig): Promise<RetentionConfig> =>
    api.put('/api/v1/system/retention', config).then(res => res.data),

  getDefaults: (): Promise<RetentionConfig> =>
    api.get('/api/v1/system/retention/defaults').then(res => res.data),
};
