  ...
  const columns: ColumnDef<Application>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const isGlobalWs = wsName === 'global'
        const displayName = isGlobalWs ? t('workspaces.globalDefault', 'Default Workspace') : wsName
        return displayName ? (
          <Badge variant="outline" className="cursor-pointer hover:bg-accent transition-colors" onClick={handleClick}>
            {displayName} <ExternalLink className="ml-1 h-3 w-3" />
          </Badge>
        ) : (
          <span className="text-muted-foreground text-xs cursor-pointer hover:underline" onClick={handleClick}>{t('workspaces.globalDefault', 'Default Workspace')}</span>
        )
      },
    },
    {
