...
import { Shield, Plus, Edit, Trash2, Globe, User, Search, Settings2, Loader2, Wand2, Play, Info, FileText, Code, ChevronDown, ChevronRight, Lock, Crown } from 'lucide-react'
...
const DataSecurity: React.FC<DataSecurityProps> = ({ workspaceId }) => {
  ...
  const formSchema = z.object({
    ...
    mask_keep_prefix: z.string().optional(),
    mask_keep_suffix: z.string().optional(),
    mask_char: z.string().optional(),
    regex_natural_desc: z.string().optional(),
    regex_pattern: z.string().optional(),
    regex_replacement: z.string().optional(),
    genai_anonymization_prompt: z.string().optional(),
    recognition_natural_desc: z.string().optional(),
    recognition_test_input: z.string().optional(),
    entity_definition_test_input: z.string().optional(),
    test_input: z.string().optional(),
    genai_code_desc: z.string().optional(),
    genai_code_test_input: z.string().optional(),
  ...
  const handleEdit = (record: EntityType) => {
    ...
    if (anonymizationMethod === 'replace') {
      formValues.replace_text = config.replacement || ''
    } else if (anonymizationMethod === 'mask') {
      formValues.mask_keep_prefix = config.keep_prefix !== undefined ? String(config.keep_prefix) : ''
      formValues.mask_keep_suffix = config.keep_suffix !== undefined ? String(config.keep_suffix) : ''
      formValues.mask_char = config.mask_char || '*'
    } else if (anonymizationMethod === 'regex_replace') {
      formValues.regex_natural_desc = config.natural_language_desc || ''
      formValues.regex_pattern = config.regex_pattern || ''
      formValues.regex_replacement = config.replacement_template || ''
    } else if (anonymizationMethod === 'genai' || anonymizationMethod === 'genai_natural') {
      formValues.genai_anonymization_prompt = config.anonymization_prompt || ''
    }
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    switch (anonymizationMethod) {
      ...
      case 'genai':
      case 'genai_natural':
        anonymization_config = { anonymization_prompt: values.genai_anonymization_prompt || '' }
        break
    }
  ...
  const handleTestRecognitionRegex = async () => {
    ...
    setRecognitionTestResult(null)
    try {
      const result = await dataSecurityApi.testRecognitionRegex({ pattern, test_input: testInput })
      if (result.success) {
        setRecognitionTestResult({ matched: result.matched, matches: result.matches })
      } else {
        setRecognitionTestResult({ matched: false, matches: [], error: result.error })
      }
    } catch (error) {
  ...
  const handleTestEntityDefinition = async () => {
    ...
    try {
      ...
      if (result.success) {
        setEntityDefinitionTestResult({ matched: result.matched, matches: result.matches })
      } else {
        setEntityDefinitionTestResult({ matched: false, matches: [], error: result.error })
      }
    } catch (error) {
  ...
  const handleTestAnonymization = async () => {
    ...
    switch (method) {
      case 'mask':
        ...
        break
      case 'replace':
        config = { replacement: form.getValues('replace_text') || `<${form.getValues('entity_type')}>` }
        break
      case 'regex_replace':
        config = {
      ...
      case 'genai':
      case 'genai_natural':
        config = { anonymization_prompt: form.getValues('genai_anonymization_prompt') }
        break
    }
    setTestingAnonymization(true)
    setTestResult(null)
    try {
      const result = await dataSecurityApi.testAnonymization({ method, config, test_input: testInput })
      if (result.success) {
  ...
  const handleTestGenaiCode = async () => {
    ...
    setGenaiCodeTestResult(null)
    try {
      const result = await dataSecurityApi.testGenaiCode({ code: generatedGenaiCode, test_input: testInput })
      if (result.success) {
        setGenaiCodeTestResult({ anonymized_text: result.anonymized_text })
      } else {
        setGenaiCodeTestResult({ anonymized_text: '', error: result.error })
      }
    } catch (error) {
  ...
  // Generate entity type code
  const handleGenerateEntityTypeCode = async (entityTypeName: string) => {
    if (!entityTypeName || entityTypeName.trim().length === 0) return
    setGeneratingEntityTypeCode(true)
    try {
      const result = await dataSecurityApi.generateEntityTypeCode({ entity_type_name: entityTypeName.trim() })
      if (result.success && result.entity_type_code) {
        form.setValue('entity_type', result.entity_type_code)
      } else {
        const fallbackCode = entityTypeName.toUpperCase().replace(/[^A-Z\s]/g, '').replace(/\s+/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')
        if (fallbackCode) form.setValue('entity_type', fallbackCode)
      }
    } catch (error) {
      const fallbackCode = entityTypeName.toUpperCase().replace(/[^A-Z\s]/g, '').replace(/\s+/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')
      if (fallbackCode) form.setValue('entity_type', fallbackCode)
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1 whitespace-nowrap">
          {row.original.check_input && <Badge variant="default" className="text-xs">{t('entityType.input')}</Badge>}
          {row.original.check_output && <Badge variant="outline" className="text-xs">{t('entityType.output')}</Badge>}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <Badge variant={is_active ? 'default' : 'outline'}>{is_active ? t('common.enabled') : t('common.disabled')}</Badge>
      },
    },
    {
      id: 'source_type',
      ...
      cell: ({ row }) => {
        const sourceType = row.original.source_type || (row.original.is_global ? 'system_template' : 'custom')
        if (sourceType === 'system_template') {
          return <Badge variant="default" className="gap-1 whitespace-nowrap"><Globe className="h-3 w-3" />{t('entityType.systemTemplate')}</Badge>
        } else if (sourceType === 'system_copy') {
          return <Badge variant="secondary" className="gap-1 whitespace-nowrap"><Globe className="h-3 w-3" />{t('entityType.systemCopy')}</Badge>
        } else {
          return <Badge variant="outline" className="gap-1 whitespace-nowrap"><User className="h-3 w-3" />{t('entityType.custom')}</Badge>
        }
      },
    },
    ...(canEdit ? [{
      ...
      cell: ({ row }: { row: any }) => {
        ...
        const canDelete = sourceType === 'system_template' ? user?.is_super_admin : sourceType === 'custom'
        return (
          <div className="flex gap-1">
            <Button variant="ghost" size="sm" onClick={() => handleEdit(record)} disabled={!canEditRow}>
              <Edit className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" onClick={() => handleDelete(record.id)} disabled={!canDelete}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xl font-semibold">{t('dataSecurity.dataLeakPrevention')}</h2>
      </div>

      {/* Entity Types Section */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">{t('entityType.entityTypeConfig')}</h3>
          {canEdit && (
        ...
        <div className="flex gap-3">
          ...
          {(searchText || riskLevelFilter) && (
            <Button variant="outline" size="sm" onClick={() => { setSearchText(''); setRiskLevelFilter(undefined) }}>
              {t('common.reset')}
            </Button>
          )}
        </div>
            ...
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="entity_type_name" render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('entityType.entityTypeNameLabel')}</FormLabel>
                      <FormControl>
                        <Input {...field} placeholder={t('entityType.entityTypeNamePlaceholder')} onBlur={(e) => { field.onBlur(); if (!editingEntity && e.target.value) handleGenerateEntityTypeCode(e.target.value) }} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input {...field} placeholder={editingEntity ? t('entityType.entityTypeCodePlaceholder') : t('entityType.entityTypeCodeAutoGenerate')} disabled={!!editingEntity || generatingEntityTypeCode} readOnly={!editingEntity} className={!editingEntity ? 'bg-secondary' : ''} />
                          {generatingEntityTypeCode && <div className="absolute right-3 top-1/2 -translate-y-1/2"><Loader2 className="h-4 w-4 animate-spin text-slate-500" /></div>}
                        </div>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField control={form.control} name="risk_level" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
                        <SelectTrigger className="w-[200px]"><SelectValue placeholder={t('entityType.riskLevelPlaceholder')} /></SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        {RISK_LEVELS.map((level) => <SelectItem key={level.value} value={level.value}>{level.label}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
              ...
              <div className="space-y-4">
                ...
                <FormField control={form.control} name="recognition_method" render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('entityType.recognitionMethodLabel')}</FormLabel>
                    <Select onValueChange={(value) => { if (value === 'genai' && !isPremiumFeatureAvailable('genai_recognition')) { toast.error(t('entityType.premiumFeatureRequired')); return } field.onChange(value) }} value={field.value}>
                      <FormControl>
                        <SelectTrigger className="w-[200px]"><SelectValue placeholder={t('entityType.recognitionMethodPlaceholder')} /></SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        ...
                        <SelectItem value="genai" disabled={!isPremiumFeatureAvailable('genai_recognition')}>
                          <span className="flex items-center gap-2">{t('entityType.recognitionMethodGenai')}{!isPremiumFeatureAvailable('genai_recognition') && <Crown className="h-3 w-3 text-amber-500" />}</span>
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                ...
                {needsSubscription() && (
                  <Alert className="bg-amber-50 border-amber-200">
                    ...
                    <AlertDescription className="text-amber-800">{t('entityType.premiumFeatureDescription')}</AlertDescription>
                  </Alert>
                )}

                {form.watch('recognition_method') === 'regex' ? (
                  <div className="space-y-4 p-4 border rounded-lg bg-muted/30">
                    <FormField control={form.control} name="recognition_natural_desc" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('entityType.recognitionNaturalDesc')}</FormLabel>
                        <div className="flex gap-2">
                          <FormControl><Input {...field} placeholder={t('entityType.recognitionNaturalDescPlaceholder')} /></FormControl>
                    ...
                    <FormField control={form.control} name="pattern" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Textarea {...field} rows={2} placeholder={t('entityType.recognitionRulePlaceholder')} className="font-mono" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="recognition_test_input" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('entityType.recognitionTestInput')}</FormLabel>
                        <div className="flex gap-2">
                          <FormControl><Input {...field} placeholder={t('entityType.recognitionTestInputPlaceholder')} /></FormControl>
                        ...
                        {recognitionTestResult.error ? (
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {recognitionTestResult.matches.map((match, idx) => <code key={idx} className="text-xs bg-emerald-500/15 text-emerald-300 px-2 py-0.5 rounded">{match}</code>)}
                            </div>
                          </div>
                        ) : (
                  ...
                  <div className="space-y-4 p-4 border rounded-lg bg-muted/30">
                    <FormField control={form.control} name="entity_definition" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('entityType.entityDefinitionLabel')}</FormLabel>
                        <FormControl><Textarea {...field} rows={3} placeholder={t('entityType.entityDefinitionPlaceholder')} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="entity_definition_test_input" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('entityType.entityDefinitionTestInput')}</FormLabel>
                        <div className="flex gap-2">
                          <FormControl><Input {...field} placeholder={t('entityType.entityDefinitionTestInputPlaceholder')} /></FormControl>
                      ...
                      <div className={`p-3 border rounded-lg ${entityDefinitionTestResult.error ? 'bg-red-500/10 border-red-500/20' : entityDefinitionTestResult.matched ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-yellow-500/10 border-yellow-500/20'}`}>
                        {entityDefinitionTestResult.error ? (
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {entityDefinitionTestResult.matches.map((match, idx) => <code key={idx} className="text-xs bg-emerald-500/15 text-emerald-300 px-2 py-0.5 rounded">{match}</code>)}
                            </div>
                          </div>
                        ) : (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-muted/30">
                  <FormField control={form.control} name="anonymization_method" render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('entityType.anonymizationMethodSelectLabel')}</FormLabel>
                      <Select onValueChange={(value) => { if ((value === 'genai_code' || value === 'genai_natural') && !isPremiumFeatureAvailable('genai_code_anonymization')) { toast.error(t('entityType.premiumFeatureRequired')); return } field.onChange(value) }} value={field.value}>
                        <FormControl>
                          <SelectTrigger className="w-[200px]"><SelectValue placeholder={t('entityType.anonymizationMethodSelectPlaceholder')} /></SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          {ANONYMIZATION_METHODS.map((method) => {
                            ...
                            return (
                              <SelectItem key={method.value} value={method.value} disabled={!isAvailable}>
                                <span className="flex items-center gap-2">{method.label}{isPremium && !isAvailable && <Crown className="h-3 w-3 text-amber-500" />}</span>
                              </SelectItem>
                            )
                          })}
                        </SelectContent>
                  ...
                  {form.watch('anonymization_method') === 'replace' && (
                    <FormField control={form.control} name="replace_text" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('entityType.replaceText')}</FormLabel>
                        <FormControl><Input {...field} placeholder={t('entityType.replaceTextPlaceholder')} /></FormControl>
                    ...
                    <div className="space-y-4">
                      <FormField control={form.control} name="regex_natural_desc" render={({ field }) => (
                        <FormItem>
                          ...
                          <div className="flex gap-2">
                            <FormControl><Input {...field} placeholder={t('entityType.regexNaturalDescPlaceholder')} /></FormControl>
                  ...
                  {form.watch('anonymization_method') === 'genai_natural' && (
                    <FormField control={form.control} name="genai_anonymization_prompt" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Textarea {...field} rows={3} placeholder={t('entityType.genaiAnonymizationPromptPlaceholder')} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  )}
                  ...
                  {form.watch('anonymization_method') === 'genai_code' && (
                    <div className="space-y-4">
                      <FormField control={form.control} name="genai_code_desc" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t('entityType.genaiCodeDesc')}</FormLabel>
                          <FormControl><Textarea {...field} rows={3} placeholder={t('entityType.genaiCodeDescPlaceholder')} /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <Button type="button" variant="outline" onClick={handleGenerateGenaiCode} disabled={generatingGenaiCode || !form.getValues('genai_code_desc')} className="w-full">
                      ...
                      {generatedGenaiCode && (
                        <>
                          <div className="border rounded-lg overflow-hidden">
                            <button type="button" onClick={() => setShowGenaiCode(!showGenaiCode)} className="w-full flex items-center justify-between p-3 bg-secondary hover:bg-card/5 transition-colors">
                              <div className="flex items-center gap-2 text-sm font-medium text-slate-300">
                                <Code className="h-4 w-4" />{t('entityType.viewGeneratedCode')}
                              </div>
                              {showGenaiCode ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
                            </button>
                            {showGenaiCode && <div className="p-3 bg-gray-900 overflow-x-auto"><pre className="text-xs text-gray-100 font-mono whitespace-pre-wrap">{generatedGenaiCode}</pre></div>}
                          </div>
                          <FormField control={form.control} name="genai_code_test_input" render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('entityType.genaiCodeTestInput')}</FormLabel>
                              <div className="flex gap-2">
                                <FormControl><Input {...field} placeholder={t('entityType.genaiCodeTestInputPlaceholder')} /></FormControl>
                  ...
                  {['hash', 'encrypt', 'shuffle', 'random'].includes(form.watch('anonymization_method') || '') && (
                    <p className="text-xs text-muted-foreground">{t('entityType.anonymizationMethodNoConfig')}</p>
                  )}

                  {form.watch('anonymization_method') !== 'genai_code' && (
                    <div className="pt-4 border-t">
                      <FormField control={form.control} name="test_input" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t('entityType.testAnonymization')}</FormLabel>
                          <div className="flex gap-2">
                            <FormControl><Input {...field} placeholder={t('entityType.testInputPlaceholder')} /></FormControl>
              ...
              <div className="space-y-4">
                ...
                {user?.is_super_admin && !editingEntity && (
