...
const FalsePositiveAppeal: React.FC<FalsePositiveAppealProps> = ({ workspaceId, embedded }) => {
  ...
  return (
    <div className={embedded ? "flex flex-col gap-4" : "flex flex-col gap-4 min-h-0 flex-1"}>
      {!embedded && <h2 className="text-2xl font-bold tracking-tight shrink-0">{t('appealConfig.pageTitle')}</h2>}

      {/* Compact Config Section */}
      <Collapsible open={configExpanded} onOpenChange={setConfigExpanded} className="shrink-0">
        <Card>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="font-medium text-muted-foreground mb-3 text-sm">
                    {t('appealConfig.originalContent')}:
                  ...
                  <div className="mt-2 p-4 bg-secondary rounded-md">
                    <p className="whitespace-pre-wrap text-sm">
                      {selectedResult?.unsafe_segments && selectedResult.unsafe_segments.length > 0
                        ? renderHighlightedContent(selectedAppeal.original_content, selectedResult.unsafe_segments)
                  ...
                  {selectedResult.hit_keywords && (() => {
                    try {
                      ...
                      if (Array.isArray(keywords) && keywords.length > 0) {
                        return (
                          <div className="grid grid-cols-3 gap-4 border-b pb-3">
                            <div className="font-medium text-muted-foreground text-sm">{t('results.hitKeywords')}:</div>
