...
public final class OgrConfig {
    ...
    /**
     * The name this integration reports as.
     *
     * <p>⚠️ Changing it MINTS A SECOND INTEGRATION RECORD rather than renaming the one
     * there is — a runtime keys liveness on the NAME so that a rollout updates its row
     * instead of reporting the old build as dark. A host that embeds this library and
     * wants its own name should set it once, at first deployment, and then leave it.
     */
    public static final String DEFAULT_INTEGRATION_NAME = "ogr-java-bridge";
