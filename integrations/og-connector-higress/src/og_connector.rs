...
pub struct OGInputResponse {
    /// Disposition action: block, anonymize, switch_private_model, replace, pass
    pub action: String,
    ...
    /// Private model info (when action = "switch_private_model")
    #[serde(default)]
    pub private_model: Option<PrivateModel>,
...
pub struct PrivateModel {
    /// API base URL for the private model
