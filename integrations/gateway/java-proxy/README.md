# The Java proxy integration

OGR for a proxy you already run. A **library** a Java service embeds, and a
**runnable reference proxy** built on it.

```
   client ──HTTP──▶  your proxy (ogr-proxy-core)  ──▶ OpenAI / Anthropic
                            │
...
java -jar server/target/ogr-proxy-server-1.0.0.jar
```
