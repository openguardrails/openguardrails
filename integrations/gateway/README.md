...
⚠️ **A proxy the organization wrote itself is not a gateway integration** — it has
no extension point of ours to plug into, and the code around it is code this
project will never see. That is [`../bridge/`](../bridge/), a category of its own.
Its [`DESIGN.md`](../bridge/java/DESIGN.md) is the protocol-conversion write-up —
what the conversion has to do, job by job, with the failure each rule prevents —
and is worth reading before building one in any language, this directory included.
