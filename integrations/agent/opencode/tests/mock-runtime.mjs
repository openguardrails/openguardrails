 ...
 * reporter's own `"name/version"`, and `session_hint`, opencode's `sessionID`.
...
function redactionProblems(r) {
  ...
  for (const m of r.masked) {
    if (typeof m?.token !== "string" || !/^(?:OGRK[0-9A-Z][0-9X]{7,}|\$\{OGR_[A-Z_]+_[0-9A-Z]+\})$/.test(m.token)) problems.push(`redaction.masked token ${JSON.stringify(m?.token)}`)
