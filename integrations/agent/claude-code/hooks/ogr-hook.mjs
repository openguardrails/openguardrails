...
// ../../../packages/javascript/dist/llm-derive.js
var MAX_TOOLS = 64;
function asDict(v) {
...
function sniffProtocol(ev) {
  const hint = ev.llmProtocol;
  if (hint === "openai.chat" || hint === "anthropic.messages")
    return hint;
  const p = ev.payload;
  const messages = asArray(p.messages);
  if (ev.kind === "llm_response") {
    if (asArray(p.choices).length > 0)
      return "openai.chat";
    if (p.type === "message" || asArray(p.content).length > 0)
      return "anthropic.messages";
    return null;
  }
  if (messages.length === 0)
    return null;
  if (typeof p.system === "string" || typeof p.system === "object")
    return "anthropic.messages";
  const tools = asArray(p.tools).map(asDict).filter(Boolean);
  if (tools.some((t) => t.input_schema !== void 0))
    return "anthropic.messages";
  if (tools.some((t) => t.function !== void 0 || t.type === "function"))
    return "openai.chat";
  const blocks = messages.some((m) => asArray(asDict(m)?.content).some((b) => {
    const t = asDict(b)?.type;
    return t === "tool_result" || t === "tool_use";
  }));
  return blocks ? "anthropic.messages" : "openai.chat";
}
function normalizeTools(proto, tools) {
  const list = asArray(tools).slice(0, MAX_TOOLS).map(asDict).filter(Boolean);
  if (list.length === 0)
    return null;
  return list.map((t) => {
...
function deriveRequest(ev, proto) {
  const p = ev.payload;
  const messages = asArray(p.messages).map(asDict).filter(Boolean);
  let system = "";
  ...
  for (const m of fresh) {
    ...
    if (m.role !== "user")
      continue;
    const blocks = asArray(m.content).map(asDict).filter(Boolean);
    for (const b of blocks.filter((b2) => b2.type === "tool_result")) {
      results.push({ tool_call_id: b.tool_use_id ?? "", result: textOf(b.content) });
    }
    const text = typeof m.content === "string" ? m.content : blocks.filter((b) => b.type === "text").map((b) => String(b.text ?? "")).join("\n");
    if (text)
      userTexts.push(text);
  }
  const payload = {};
...
function deriveResponse(ev, proto) {
  const p = ev.payload;
  const payload = {};
  const calls = [];
  if (proto === "openai.chat") {
    ...
    const blocks = asArray(p.content).map(asDict).filter(Boolean);
    payload.text = blocks.filter((b) => b.type === "text").map((b) => String(b.text ?? "")).join("\n");
...
function deriveLlmEvent(ev) {
  if (ev.kind !== "llm_request" && ev.kind !== "llm_response")
    return;
  try {
    ...
    if (ev.kind === "llm_request")
      deriveRequest(ev, proto);
    else
      deriveResponse(ev, proto);
  } catch {
    ;
    ev.payload.unparsed = true;
  }
...
// hooks/ogr-hook.src.mjs
var AGENT_ID = process.env.OGR_AGENT_ID || `claude-code-${hostname()}`;
