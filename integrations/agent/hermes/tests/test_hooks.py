...
def test_the_outbound_request_is_masked_before_it_leaves(protected):
    ...
    assert req["messages"][0]["content"] == "Use the key OGRKP0000001 for the API."
...
def test_the_step_request_event_carries_what_this_step_minted(protected):
    ...
    assert ev["redaction"] == {
        ...
        "masked": [{"token": "OGRKP0000001", "rule": "entity_api_key/openai"}],
    }
    ...
    assert req2["messages"][0]["content"] == "Still OGRKP0000001"
...
def test_the_ogr_client_is_an_egress_too(protected):
    ...
    _round(protected, text=f"Try Authorization: Bearer {BEARER_KEY}",
           tool_calls=[FakeToolCall("c1", "bash", '{"command": "curl -H \'Authorization: Bearer OGRKP0000001\'"}')])
    ...
    assert ev["payload"]["tool_calls"][0]["arguments"]["command"] \
        == "curl -H 'Authorization: Bearer OGRKP0000001' https://x"
...
def test_restore_happens_only_in_tool_execution_and_after_judgement(protected):
    ...
    args = {"command": "curl -H 'Authorization: Bearer OGRKP0000001' https://api"}
    ...
    assert args["command"].count("OGRKP0000001") == 1      # caller's dict untouched


def test_an_unrestorable_token_blocks_the_call_at_both_seams(protected):
    _masked_request()
    args = {"command": "echo OGRKP0000007"}
    out = bridge.on_pre_tool_call(tool_name="bash", args=args, session_id="s-1",
                                  tool_call_id="c1")
    assert out["action"] == "block"
    assert "OGRKP0000007 could not be restored" in out["message"]
    called = []
    ...
    assert "OGRKP0000007 could not be restored" in res
    ...
    out = bridge.on_pre_tool_call(tool_name="bash", args={"c": "OGRKP0000001"},
...
def test_the_minted_token_restores_into_the_tool_exactly_as_written(protected):
    ...
    bridge.on_tool_execution_middleware(
        tool_name="bash", args={"command": "echo OGRKP0000001"},
...
def test_the_final_answer_keeps_tokens_unless_restore_output_is_on(protected, clean_env):
    _masked_request()
    _round(protected, text="The key is OGRKP0000001.")
    assert bridge.on_transform_llm_output(response_text="The key is OGRKP0000001.",
                                          session_id="s-1") is None
