...
test("round trip: a masked value restores into a tool's arguments, and only there", () => {
  ...
  assert.equal(out.text, "export AWS_ACCESS_KEY_ID=OGRKP0000001")
  // The model, having seen the token, uses it in a call.
  const args = { command: "aws s3 ls", env: { AWS_ACCESS_KEY_ID: "OGRKP0000001" }, list: ["OGRKP0000001", 3, null] }
  ...
  assert.equal(args.env.AWS_ACCESS_KEY_ID, "OGRKP0000001")
})

test("restoreArgs reports every unresolved token once and leaves the args untouched", () => {
  const map = new SessionMap("s1")
  const args = { a: "OGRKP0000004", b: ["OGRKP0000004", "OGRKPXXXXXXX"] }
  const r = restoreArgs(args, map)
  assert.equal(r.args, args)
  assert.equal(r.changed, false)
  assert.deepEqual(r.unresolved.sort(), ["OGRKP0000004", "OGRKPXXXXXXX"])
  assert.match(UNRESTORABLE_NOTICE("OGRKP0000004"), /^OGRKP0000004 could not be restored: it is not a placeholder this session issued\./)
})
...
test("value stability across steps and across the walk", () => {
  ...
  assert.equal(a.text, "first OGRKP0000001")
  assert.equal(b.value.messages[0].content, "again OGRKP0000001")
...
test("the bound: value 257 is masked with the non-restorable overflow token and warned once", () => {
  ...
  assert.equal(r.text, `\OGRKP0000001 \OGRKP0000002 \OGRKP0000003 ${OVERFLOW_TOKEN} ${OVERFLOW_TOKEN}`)
...
test("the request walk keeps structure, indexes and structural keys", () => {
  ...
  assert.equal(out.messages[0].content, "You hold OGRKP0000001.")
  assert.equal(out.messages[1].content[0].text, "use OGRKP0000002")
  ...
  assert.equal(out.messages[2].tool_calls[0].function.arguments, '{"command":"echo OGRKP0000001"}')
  assert.equal(out.messages[3].content, "OGRKP0000001\n")
  assert.equal(out.tools[0].function.description, "never print OGRKP0000001")
  assert.equal(out.temperature, 0.2)
  assert.equal(out.model, "gpt-4.1")
  assert.deepEqual(r.minted.map((m) => m.token), ["OGRKP0000001", "OGRKP0000002"])
...
test("a Responses-style input and an anthropic system block are leaves too", () => {
  ...
  assert.equal(r.value.system[0].text, "key OGRKP0000001")
  assert.equal(r.value.input[0].content[0].text, "and OGRKP0000001")
})
...
test("maskKnown runs no rules: the egress pass replaces only what the session already holds", () => {
  ...
  assert.equal(r.value.tool_calls[0].arguments.command, `echo \OGRKP0000001 ${OPENAI}`)
...
test("zero-width characters inside a token do not open it, and the splice keeps the rest of the text byte-exact", () => {
  ...
  assert.equal(r.text, "​pre OGRKP0000001 post​")
  assert.equal(map.valueOf("OGRKP0000001"), AWS)
})
