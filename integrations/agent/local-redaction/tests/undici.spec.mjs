...
test("undici dispatcher half: a captured fetch and undici.request are both covered", { skip: !undici && "undici is not resolvable here" }, async () => {
  ...
  const server = createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      res.end(JSON.stringify({ choices: [{ message: { content: "OGRKP0000001", tool_calls: [{ function: { name: "bash", arguments: '{"cmd":"OGRKP0000001"}' } }] } }] }))
    })
  })
  ...
  try {
    ...
    assert.equal(JSON.parse(received[0].raw).messages[0].content, "k OGRKP0000001")
    assert.equal(received[0].headers["accept-encoding"], "identity")
    assert.equal(reply.choices[0].message.content, "OGRKP0000001")
...
test("both halves on: a request through the wrapped fetch is masked once and counted once — the undici half recognises it by its body", { skip: !undici && "undici is not resolvable here" }, async () => {
  ...
  const server = createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      res.writeHead(200, { "content-type": "application/json" })
      res.end(JSON.stringify({ choices: [{ message: { content: "ok", tool_calls: [{ function: { name: "bash", arguments: '{"cmd":"OGRKP0000001"}' } }] } }] }))
    })
  })
  await new Promise((r) => server.listen(0, "127.0.0.1", r))
  const base = `http://127.0.0.1:${server.address().port}`
