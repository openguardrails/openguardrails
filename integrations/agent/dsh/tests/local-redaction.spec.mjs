...
test("the runtime is shown a token, never the key", async () => {
  await withCache(() =>
    withRuntime(boot, {}, () => "allow", async ({ stream, runtime }) => {
      ...
      assert.match(sent, /OGRKP0000001/, "the key was not replaced by a placeholder")
    }, { rules: TEST_RULESET }))
})
  ...
  await withCache(() =>
    withRuntime(boot, {}, () => "allow", async ({ stream, runtime }) => {
      ...
      assert.deepEqual(request.redaction.masked, [
        { token: "OGRKP0000001", rule: "entity_api_key/openai_project" },
      ])
...
test("one value, one token — across steps of the same session", async () => {
  await withCache(() =>
    withRuntime(boot, {}, () => "allow", async ({ stream, runtime }) => {
      ...
      const tokens = requests.map((e) => JSON.stringify(e.payload).match(/OGRK[A-Z]\d{7}/)[0])
