 ...
 */
export const INTEGRATION = "ogr-openclaw/0.4.0"
...
export interface WireEvent {
  ...
  integration?: string
  /**
   * WHICH CONVERSATION — OpenClaw's own `sessionKey` for the session this held
   * action belongs to. A grouping HINT, not a coordinate: the runtime still
   ...
   * when local redaction is off; `ruleset: ""` when it is on but no ruleset
   * was ever obtained.
   */
